...
const ProductCard = ({ product, showBuy = false }) => {
  ...
  return (
    <div
      ...
      className="
        relative bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-md 
        hover:shadow-2xl transition-all duration-500 ease-out cursor-pointer
        w-full sm:max-w-[250px] md:max-w-[300px] lg:max-w-[330px] 
        h-[400px] sm:h-[380px] md:h-[400px] flex flex-col group hover:-translate-y-2
      "
    >
      <div className="overflow-hidden relative bg-gray-50 h-48 sm:h-44 md:h-56">
        <img
          src={imgSrc}
          alt={product.title}
          ...
          loading="lazy"
          onError={(e) => {
            if (!e.currentTarget.src.includes("placeholder.com")) {
        ...
        <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
          <button
            ...
            className="bg-white text-black p-3 sm:p-4 rounded-full shadow-xl hover:bg-gray-200 transition-all duration-300 scale-90 group-hover:scale-110"
            title="Add to cart"
          >
      ...
      <div className="p-4 sm:p-5 flex flex-col justify-between flex-grow">
        <div>
          <h3 className="text-gray-900 text-lg sm:text-xl font-bold leading-snug mb-1 capitalize truncate h-10 overflow-hidden">
            {truncateWords(product.title, 3)}
          </h3>
          <p className="text-gray-600 text-sm sm:text-base mb-3 leading-tight font-medium h-16 overflow-hidden">
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-200">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-black hover:bg-gray-800 text-white text-xs sm:text-sm font-semibold px-3 sm:px-4 py-2 rounded-xl transition-all duration-300 shadow-lg hover:shadow-2xl focus:outline-none focus:ring-4 focus:ring-gray-400/40"
            title="Buy Now"
          >
            <Zap size={18} /> Buy Now
          </button>
