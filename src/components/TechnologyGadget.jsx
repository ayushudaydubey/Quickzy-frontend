...
// Skeleton Loader (light gray tech theme)
const Skeleton = () => (
...
// Navigation Buttons (animated + dark)
const NavButton = ({ direction, onClick }) => (
  ...
  >
    ...
    <div className="absolute inset-0 bg-gradient-to-r from-zinc-900 via-zinc-700 to-black opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-full pointer-events-none" />

    {/* Shimmer effect */}
    <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/20 to-transparent rounded-full pointer-events-none" />
...
const Technology = () => {
  ...
  useEffect(() => {
    axiosInstance
      .get("/products")
      .then((res) =>
        setProducts(res.data.filter((p) => p.category === "Technology"))
      )
      ...
      <div className="flex items-end justify-between mb-10 py-6 px-4 rounded-2xl  bg-gradient-to-r from-zinc-100/70 via-zinc-500 to-zinc-700">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="relative px-6 py-2.5 bg-gradient-to-r from-zinc-900 via-zinc-800 to-black text-white text-[11px] font-medium tracking-[0.15em] uppercase rounded-full transition-all duration-500 shadow-sm hover:shadow-xl overflow-hidden group"
          >
            {/* Animated gradient background */}
            <div className="absolute inset-0 bg-gradient-to-r from-zinc-700 via-zinc-800 to-zinc-900 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        ...
        >
          {loading
            ...
            : products.map((product) => (
                <div
                  key={product._id}
                  className="group min-w-[300px] bg-zinc-100 rounded-2xl overflow-hidden border border-zinc-300 hover:border-zinc-400 transition-all mt-4 duration-300 hover:scale-[1.02]"
                >
